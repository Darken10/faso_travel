<div class="lg:w-2/6 w-full" >
    <div class="w-full  p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
        <form class="space-y-6" wire:submit="handleSubmite" action="{{ route('ticket.payer-with-voyage-instance',$voyage)  }}" method="POST" >
            @csrf
            <h5 class="text-2xl font-bold text-gray-900 dark:text-white">Acheter le Ticket</h5>

            <div class="md:flex gap-4 items-center">
                <div class="w-full">
                    <x-label for="voyage_instance_choise" value="Date du voyage" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" />
                    <x-select wire:change.debounce="handlerDateOnChange"  wire:model="voyage_instance_choise"  id="voyage_instance_choise" name="voyage_instance_choise" type="text" class="mt-1 block w-full" autofocus  value="{{ old('date') }}" >
                        @foreach($voyage->voyage_instances as $v_i)
                            <option value="{{ $v_i->id }}">{{ $v_i->date->format("l d M Y") }}</option>
                        @endforeach
                    </x-select>
                    ...
                    <x-input-error for="voyage_instance_choise" />
                </div>
            </div>
